...
def p_method_decl_header_void(p):
    ...
    global current_context, current_vartable, current_method_return_type
    current_context = 'method'
    (v, s) = current_modifiers
    m = ast.Method(p[3], current_class, v, s, ast.Type('void'))
    current_method_return_type = ast.Type('void')
...
def p_method_decl_header_nonvoid(p):
    ...
    current_method_return_type = current_type
    current_class.add_method(m)
    current_vartable = m.vars
    p[0] = m
...
def p_stmt_return(p):
    ...
    p[0].populate_current_method_return_type(current_method_return_type)
