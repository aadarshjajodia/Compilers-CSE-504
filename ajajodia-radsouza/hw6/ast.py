...
def generate_mips():
    global asmfile
    asmfile = open("input.asm", "w")
    asmfile.write("\t.text\n")
    asmfile.write("\t.align 4\n") 
    asmfile.write("\t.globl main\n") # TBD: name should be main_entry_point
    for cid in classtable:
        c = classtable[cid]
        c.generate_mips()
    if staticfieldcount:
        asmfile.write("\t.data\n")
        asmfile.write("static_data: .space {0}\n".format(str(staticfieldcount*4)))
...
class Instruction:
    def __init__(self, name, defvars, usevars, remainingvars, label = "", is_part_of_liveness_analysis = True):
        self.name = name
        self.defined = []
        if(defvars != ''):
            self.defined.append(defvars)
        self.uselist = usevars
        self.remainingvars = remainingvars
        self.successors = []
        self.label = label
        self.is_part_of_liveness_analysis = is_part_of_liveness_analysis
        self.instructionLabel = []

    def add_successor(self, successor):
        self.successors.append(successor)

    def add_instruction_label(self, label):
        self.instructionLabel.append(label)

    def printout(self):
        if self.instructionLabel != "":
            print self.instructionLabel,
        print self.name, self.defined, self.uselist, self.remainingvars,
        print "successors is:",
...
class Method:
    ...
    def generate_code(self):
        global current_method, current_instruction, label_instruction_dict
        current_method = self
        current_instruction = None
        label_instruction_dict = {}
        if self.name == "main":
        ...
        for x in self.instructions:
            if x.label != "":
                x.add_successor(label_instruction_dict[x.label])

        #Poulating Label Names for Instruction
        for key, value in label_instruction_dict.iteritems():
            self.instructions[value].add_instruction_label(key)
        
        return self.perform_liveness_analysis()

    def generate_mips(self):
        if self.name == "main":
            asmfile.write("main: # this is my main_entry_point\n") # TBD: name should be main_entry_point
        else:
            asmfile.write("M_{0}_{1}:\n".format(self.name, str(self.id)))
        generate_mips_code(self.instructions, self.name)
        asmfile.write("\n")
    ...
    def perform_liveness_analysis(self):
        ...
        #    x.printout()
        index1 = 0
        tempinstructions = self.instructions

        for x in range(0, len(tempinstructions)):
            Insets.append(Set())
            Outsets.append(Set())
        
        continue_liveness = True
        while continue_liveness == True:
            continue_liveness = False
            for x in range(len(tempinstructions)-1, -1, -1):
                regex = re.compile(r'^t.*')
                tempuselist = tempinstructions[x].uselist
                tempuselist = filter(regex.match, tempuselist)

                tempdefinedlist = tempinstructions[x].defined
                tempdefinedlist = filter(regex.match, tempdefinedlist)

                Nodes = Nodes | Set(tempuselist) | Set(tempdefinedlist)
                prevInSet = Insets[x]
                prevOutSet = Outsets[x]
                if tempinstructions[x].is_part_of_liveness_analysis == True:
                    for index,succ in enumerate(tempinstructions[x].successors):
                        Outsets[x] = Outsets[x] | Insets[succ]
                    Insets[x] = Set(tempuselist) | (Outsets[x] - Set(tempdefinedlist))
                if(prevInSet != Insets[x] or prevOutSet != Outsets[x]):
                    continue_liveness = True
        ...
        if(len(Nodes) == 0):
            return True
        #    for x in range(0, len(tempinstructions)):
        #        print "Inset", Insets[x], "Outset", Outsets[x]
            ...
            if len(list1) > 1:
                for p in range(0, len(list1)):
                    for q in range(p+1, len(list1)):
                        if list1[p] not in neighbors.keys():
                            neighbors[list1[p]] = []
                        if list1[q] not in neighbors.keys():
                            neighbors[list1[q]] = []
                        neighbors[list1[p]].append(list1[q])
                        neighbors[list1[q]].append(list1[p])

        if(len(self.instructions) > 0):
            register_map = graph_color.graph_color(Nodes, neighbors)
            #print register_map
            if register_map:
                for inst in self.instructions:
                    for index in range(0, len(inst.defined)):
                        if inst.defined[index] in register_map.keys():
                            inst.defined[index] = register_map[inst.defined[index]]
                    for index in range(0, len(inst.uselist)):
                        if inst.uselist[index] in register_map.keys():
                            inst.uselist[index] = register_map[inst.uselist[index]]
            else:
                print "Unable to fit these registers in memory, Not generating MIPS code"
                return False
            index = 0
            while index < len(self.instructions):
                if self.instructions[index].name == "move":
                    if self.instructions[index].defined[0] == self.instructions[index].uselist[0]:
        ...
        save_optimization_map = {}
        save_list = []
        index = 0
        while index < len(self.instructions):
            if self.instructions[index].name == "save":
                if self.instructions[index].uselist[0] in save_optimization_map:
                    self.instructions.pop(index)
                    continue
                else:
                    save_optimization_map[self.instructions[index].uselist[0]] = 1
                    save_list.append(self.instructions[index].uselist[0])
            elif self.instructions[index].name == "restore":
                #print "save_list", save_list
                save_optimization_map = {}
                if len(save_list) > 0:
                    self.instructions[index].uselist[0] = save_list.pop()
                else:
                    self.instructions.pop(index)
                    continue
            index += 1
...
class IfStmt(Stmt):
    ...
    def generate_code(self):
        global labelnumber, current_instruction
        elselabel = "L" + str(labelnumber)
        labelnumber = labelnumber + 1
        tempreg = self.condition.generate_code()
        amifile.write("\tbz {0}, {1}\n".format(tempreg, elselabel))

        bzinst = Instruction("bz", "", [tempreg], [elselabel])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        # 2 successors for bz instruction
        #
        current_instruction = bzinst
        current_method.instructions.append(bzinst)
        ...
        if not isinstance(self.elsepart, SkipStmt):
            ...
            inst = Instruction("jmp", "", emptyList, [thenlabel])
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = None
            current_method.instructions.append(inst)

            label_instruction_dict[elselabel] = len(current_method.instructions)
            amifile.write("{0}:\n".format(elselabel))
            self.elsepart.generate_code()
            amifile.write("{0}:\n".format(thenlabel))
            label_instruction_dict[thenlabel] = len(current_method.instructions)
            inst.add_successor(label_instruction_dict[thenlabel])
        else:

            amifile.write("{0}:\n".format(elselabel))
            label_instruction_dict[elselabel] = len(current_method.instructions)
        bzinst.add_successor(label_instruction_dict[elselabel])
...
class WhileStmt(Stmt):
    ...
    def has_return(self):
        ...
        if (self.body.has_return() > 0):
            ...
            return 0
    def generate_code(self):
        global labelnumber, breakstatementlabel, continuestatementlabel, current_instruction
        ...
        label_instruction_dict[continuestatementlabel] = len(current_method.instructions)

        # Adding a label instruction
        #inst = Instruction("label", "", emptyList, [continuestatementlabel])
        #if current_instruction:
        #    current_instruction.add_successor(len(current_method.instructions))
        #current_instruction = inst
        #current_method.instructions.append(inst)

        tempreg = self.cond.generate_code()
        amifile.write("\tbz {0}, {1}\n".format(tempreg, afterWhileStatementlabel))

        # Add instruction to function list
        bzinst = Instruction("bz", "", [tempreg], [breakstatementlabel])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))

        # 2 successors for bz instruction
        bzinst.add_successor(label_instruction_dict[continuestatementlabel])
        #current_method.instructions[label_instruction_dict[continuestatementlabel]].add_instruction_label(continuestatementlabel)
        current_instruction = bzinst
        current_method.instructions.append(bzinst)
        ...
        # Add instruction to function list
        inst = Instruction("jmp", "", emptyList, [whilestatementlabel])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        inst.add_successor(label_instruction_dict[continuestatementlabel])
        current_instruction = None
        current_method.instructions.append(inst)

        amifile.write("{0}:\n".format(afterWhileStatementlabel))
        label_instruction_dict[breakstatementlabel] = len(current_method.instructions)

        # Add instruction to function list
        #inst = Instruction("label", "", emptyList, [breakstatementlabel])
        #if current_instruction:
        #    current_instruction.add_successor(len(current_method.instructions))
        #current_instruction = inst
        #current_method.instructions.append(inst)
...
class ForStmt(Stmt):
    ...
    def has_return(self):
        ...
        if (self.body.has_return() > 0):
            ...
            return 0
    def generate_code(self):
        global breakstatementlabel, continuestatementlabel, current_instruction
        ...
        bzinst = Instruction("bz", "", [r1], [breakstatementlabel])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        # 2 successors for bz instruction
        bzinst.add_successor(label_instruction_dict[continuestatementlabel])
        ...
        inst = Instruction("jmp", "", emptyList, [forconditionlabel])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        inst.add_successor(label_instruction_dict[continuestatementlabel])
        current_instruction = None
...
class ReturnStmt(Stmt):
    ...
    def generate_code(self):
        ...
        if self.expr:
            ...
            #Add Instruction
            inst = Instruction("move", "a0", [r1], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
        ...
        amifile.write("\tret\n")
        #Add Instruction
        inst = Instruction("ret", "", emptyList, emptyList)
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
...
class BreakStmt(Stmt):
    ...
    def generate_code(self):
        ...
        # Add instruction to function list
        inst = Instruction("jmp", emptyList, emptyList, emptyList, breakstatementlabel)
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = None
        current_method.instructions.append(inst)
...
class ContinueStmt(Stmt):
    ...
    def generate_code(self):
        ...
        # Add instruction to function list
        inst = Instruction("jmp", emptyList, emptyList, emptyList)
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        inst.add_successor(label_instruction_dict[continuestatementlabel])
        current_instruction = None
...
class ConstantExpr(Expr):
    ...
    def generate_code(self):
        ...
        if (self.kind == 'int'):
            ...
            # Add instruction to function list
            remainingvarsList = []
            remainingvarsList.append(self.int)
            inst = Instruction("move_immed_i", self.register, emptyList, remainingvarsList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
...
class BinaryExpr(Expr):
    ...
    def generate_code(self, callContext = None):
        ...
        instruction_name = "i" + self.bop
        if self.bop == 'and':
            amifile.write("\timul {0}, {1}, {2}\n".format(r1, r2, r3))
            instruction_name = "imul"
        elif self.bop == 'or':
            amifile.write("\tiadd {0}, {1}, {2}\n".format(r1, r2, r3))
            instruction_name = "iadd"
            ...
            inst = Instruction("igeq", r1, [r2, r3], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
            ...
            #Adding Instruction to list
            inst = Instruction("ileq", r4, [r2, r3], emptyList)
            current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
            ...
            #Adding Instruction to list
            inst = Instruction("imul", r5, [r1, r4], emptyList)
            current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
        ...
        inst = Instruction(instruction_name, r1, [r2, r3], emptyList)
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
...
class AssignExpr(Expr):
    ...
    def generate_code(self):
        ...
        if isinstance(self.lhs, ArrayAccessExpr):
            ...
            tempreg2 = self.lhs.generate_code()
            amifile.write("\tmove {0}, {1}\n".format(tempreg2, tempreg))

            #Add Instruction
            inst = Instruction("move", tempreg2, [tempreg], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
...
class AutoExpr(Expr):
    ...
    def generate_code(self):
        ...
        if(self.when == 'post'):
            ...
            #Add Instruction
            inst = Instruction("move", reg2, [reg], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
...
def generate_mips_code(instructions, name):
    offset = 40

    if name != "main":
        asmfile.write("\tsubu $sp, $sp, 32\n")
        asmfile.write("\tsw $ra, 28($sp)\n") 
        asmfile.write("\tsw $fp, 24($sp)\n") 
        asmfile.write("\taddu $fp, $sp, 32\n")

    if instructions != None:
        for index, k in enumerate(instructions):
            inst = mips_instruction_map[k.name]

            if len(k.instructionLabel) > 0:
                for l in k.instructionLabel:
                    asmfile.write("{0}:\n".format(l))

            # some optimization to use idempotent instructions
            #if inst == "li" and index < len(instructions) and (instructions[index+1].name == "hstore" or instructions[index+1].name == "hload"):
             #   continue

            if inst == "move" and index < len(instructions) and instructions[index+1].name == "ret" and name != "main":
                if k.uselist[0] != "v0":
                    asmfile.write("\tmove $v0, ${0}\n".format(k.uselist[0]))
                    continue           

            #print index, inst, k.defined, k.uselist, k.remainingvars, len(k.uselist)

            if (inst == "sw" or inst == "lw") and offset > 0:
                if inst == "lw": 
                    offset += 4  
                k.uselist.append( str(offset) + "($sp)")
                if inst == "sw": 
                    offset -= 4  
            ...
            if inst == "halloc":
                asmfile.write("\tmul ${0}, ${0}, 4\n".format(k.uselist[0]))
                asmfile.write("\tsw $a0, 20($sp)\n")
                asmfile.write("\tmove $a0, ${0}\n".format(k.uselist[0]))
                asmfile.write("\tli $v0, 9\n")
                asmfile.write("\tsyscall\n")
                asmfile.write("\tlw $a0, 20($sp)\n")
                asmfile.write("\tmove ${0}, $v0\n".format(k.defined[0]))
                continue

            if inst == "hstore":
                if k.defined[0] == "sap":
                    asmfile.write("\tla ${0}, static_data\n".format(k.uselist[0]))
                    asmfile.write("\tsw ${0}, {2}(${1})\n".format(k.uselist[1],k.uselist[0],str(int(k.remainingvars[0])*4)))
                else:
                    asmfile.write("\tsw ${0}, {2}(${1})\n".format(k.uselist[1],k.defined[0],str(int(k.remainingvars[0])*4)))
                continue

            if inst == "hload":
                if k.uselist[0] == "sap":
                    asmfile.write("\tla ${0}, static_data\n".format(k.uselist[1]))
                    asmfile.write("\tlw ${0}, {2}(${1})\n".format(k.defined[0],k.uselist[1],str(int(k.remainingvars[0])*4)))
                else:
                    asmfile.write("\tlw ${0}, {2}(${1})\n".format(k.defined[0],k.uselist[0],str(int(k.remainingvars[0])*4)))
                continue

            asmfile.write("\t{0} ".format(inst))
            for index, d in enumerate(k.defined):
                if len(k.uselist) != 0 or len(k.remainingvars) != 0 or index != (len(k.defined) - 1):
                    asmfile.write("${0}, ".format(d))
                else:
                    asmfile.write("${0}".format(d))
            for index, u in enumerate(k.uselist):
                if len(k.remainingvars) != 0 or index != (len(k.uselist) - 1):
                    asmfile.write("${0}, ".format(u))
                else:
                    if inst == "sw" or inst == "lw":
                        asmfile.write("{0}".format(u))
                    else:
                        asmfile.write("${0}".format(u))
            for index, r in enumerate(k.remainingvars):
                if  index != (len(k.remainingvars) - 1):
                    asmfile.write("{0}, ".format(r))
                else:
                    asmfile.write("{0}".format(r))
            asmfile.write("\n")
    ...
    if name == "main":
        ...
        asmfile.write("\tlw $ra, 28($sp)\n") 
        asmfile.write("\tlw $fp, 24($sp)\n") 
        asmfile.write("\taddu $sp, $sp, 32\n") 
        asmfile.write("\tjr $ra\n")
...
class FieldAccessExpr(Expr):
    ...
    def generate_code(self, isrvalue = True, reg = None):
        ...
        #Add Instruction
        inst = Instruction("move_immed_i", p1, emptyList, [str(self.field.fieldindex)])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
        ...
        if isrvalue == True:
            ...
            #Add Instruction
            inst = Instruction("hload", p2, [reg3, p1], [self.field.fieldindex])
            current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
            ...
            #Add Instruction
            inst = Instruction("hstore", reg3, [p1, reg, reg3], [self.field.fieldindex])
            current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
...
class MethodInvocationExpr(Expr):
    ...
    def generate_code(self):
        global argumentregisternumber, temporaryregisternumber, current_instruction
        ...
        while tempCount < argumentregisternumber:
            ...
            inst = Instruction("save", "", [argreg], emptyList,"", False)
            current_method.instructions.append(inst)
        ...
        while tempCount < temporaryregisternumber:
            ...
            inst = Instruction("save", "", [tempreg], emptyList,"", False)
            current_method.instructions.append(inst)
        ...
        if not isinstance(self.base, ClassReferenceExpr) \
            ...
            tempreg = self.base.generate_code()
            amifile.write("\tmove a{0}, {1}\n".format(str(currentarg), tempreg))
            inst = Instruction("move", "a" + str(currentarg), [tempreg], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
        ...
        inst = Instruction("call", "", emptyList, ["M_"+ self.mname + "_" + str(self.method.id)], "" , False)
        current_method.instructions.append(inst)

        new_tempreg = get_new_temporary_register()
        amifile.write("\tmove {0}, {1}\n".format(new_tempreg, "v0"))
               
        while len(tempregstack) > 0:
            temp = tempregstack.pop()
            amifile.write("\trestore {0}\n".format(temp))

            inst = Instruction("restore", "", [temp], emptyList,"", False)
            current_method.instructions.append(inst)

        inst = Instruction("move", str(new_tempreg), ["v0"], emptyList)
        current_method.instructions.append(inst) 
        ...
        while len(argregstack) > 0:
            ...
            inst = Instruction("restore", "", [arg], emptyList,"", False)
            current_method.instructions.append(inst)
...
class NewObjectExpr(Expr):
    ...
    def generate_code(self):
        ...
        #Add Instruction
        inst = Instruction("move_immed_i", "t" + str(temporaryregisternumber), emptyList, [self.classref.fieldindex])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
        ...
        #print "temporaryregisternumber", temporaryregisternumber
        # adding halloc instruction
        inst = Instruction("halloc", "t" + str(temporaryregisternumber), ["t" + str(temporaryregisternumber - 1)], emptyList)
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
        ...
        while tempCount <= count1:
            ...
            inst = Instruction("save", "", ["a" + str(tempCount)], emptyList,"", False)
            current_method.instructions.append(inst)
        ...
        while tempCount <= count:
            ...
            inst = Instruction("save", "", ["t" + str(tempCount)], emptyList,"", False)
            current_method.instructions.append(inst)
            tempCount = tempCount + 1

        r1 = "t" + str(count+1)
        amifile.write("\tmove a0, t{0}\n".format(str(count+1)))
        inst = Instruction("move", "a0", ["t" + str(count + 1)], emptyList)
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
        ...
        for v in self.args:
            ...
            inst = Instruction("move", "a" + str(currentarg+1), [str(tempreg)], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
        ...
        inst = Instruction("call", "", emptyList, ["C_" + str(self.constructor.id)], "" , False)
        current_method.instructions.append(inst)

        while count >= 0:
            amifile.write("\trestore t{0}\n".format(str(count)))
            inst = Instruction("restore", "", ["t" + str(count)], emptyList,"", False)
            current_method.instructions.append(inst)
            count = count - 1

        while count1 >= 0:
            amifile.write("\trestore a{0}\n".format(str(count1)))
            inst = Instruction("restore", "", ["a" + str(count)], emptyList,"", False)
            current_method.instructions.append(inst)
            count1 = count1 - 1
...
class ArrayAccessExpr(Expr):
    ...
    def generate_code(self, isrvalue = True, reg1 = None):
        ...
        #Add Instruction
        inst = Instruction("move_immed_i", temp_reg, emptyList, [index])
        if current_instruction:
            current_instruction.add_successor(len(current_method.instructions))
        current_instruction = inst
        current_method.instructions.append(inst)
        ...
        if isrvalue == False:
            ...
            #Add Instruction
            inst = Instruction("hstore", reg, [temp_reg, reg1, reg], [str(index)])
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
            ...
            #Add Instruction
            inst = Instruction("hload", p2, [reg, temp_reg], [str(index)])
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
...
class NewArrayExpr(Expr):
    ...
    def generate_code(self):
            global temporaryregisternumber, current_instruction
            ...
            for v in self.args:
                ...
                if firstCell:
                    ...
                    #Add Instruction
                    inst = Instruction("imul", tempreg, [tempreg, temp1], emptyList)
                    if current_instruction:
                        current_instruction.add_successor(len(current_method.instructions))
                    current_instruction = inst
                    current_method.instructions.append(inst)
            ...
            #Add Instruction
            inst = Instruction("halloc", "t" + str(temporaryregisternumber), [str(tempreg)], emptyList)
            if current_instruction:
                current_instruction.add_successor(len(current_method.instructions))
            current_instruction = inst
            current_method.instructions.append(inst)
